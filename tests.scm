...
;expect #f
(1 2)
;expect Error: int is not callable
...
(f 2)
;expect 4.718281828
(define (two x) (+ x 1)(+ x 2))
;expect two
(two 1)
;expect 3
((define (error x)(* (- x 1) x)) 2)
;expect Error: str is not callable

;;;problem 7
(quote (bagel (with (cream. (4 . cheese)))))
;expect (bagel (with (cream. (4 . cheese))))
(cdr '(1 (3 . 4)))
;expect ((3 . 4))

;;;problem 8
(begin (= 1 2)(print 'haha))
;expect haha
(begin (/ 1 0)(+ 1 2))
;expect Error: division by zero
(define x (begin (display 3)(/ 4 2)(* 8 3)))
;expect 3x
(/ x 4)
;expect 6
...
;;;problem 10
(define (area-of-equtriangle x)(* (/ (expt 3 0.5) 2) x x))
;expect area-of-equtriangle
(area-of-equtriangle 2)
;expect 3.4641016151377544
area-of-equtriangle
;expect (lambda (x) (* (/ (expt 3 0.5) 2) x x))
(define (area x y) (* x y))
;expect area
(area 2 3)
;expect 6
...
(define f ( mu() (+ a b)))
;expect f
(define g(lambda() (define a 5)(define b 10)(f)))
;expect g
(g)
;expect 15
(define two 2)
;expect two
(define h (mu(one)(+ one two)))
;expect h
(define k (lambda (one two)(h (* one one))))
;expect k
(k 2 two)
;expect 6
(k 3 4)
